...
const argv = yargs
  .option('dir', {
    description: 'Path to ./meta.json and ./font-build.json and ./svg folder with SVG files',
    default: './node_modules/@mdi/svg',
    type: 'string'
  })
  .option('meta', {
  ...
  .option('subset', {
    description: 'Path to ./subset.json',
    default: './subset.json',
    type: 'string'
  })
  .option('font', {
...
const distFolder = path.resolve(currentPath, argv.dist)
let metaFile = path.resolve(currentPath, argv.dir, 'meta.json')
let subsetFile = path.resolve(currentPath, 'subset.json')
...
if (argv.subset !== './subset.json') {
  // Use --subset ./otherfile.json
  subsetFile = path.resolve(currentPath, argv.subset)
}
...
if (!fs.existsSync(subsetFile)) {
  console.error(`Unable to find "${subsetFile}". Use --help to learn more.`)
  process.exit(1)
}
...
// get our subset, a simple array of icon names
const subsetString = fs.readFileSync(subsetFile)
const subsetArray = JSON.parse(subsetString)
// filter the full original meta.json by our selection
const metaString = fs.readFileSync(metaFile)
const metaJson = JSON.parse(metaString).filter(icon =>
  subsetArray.includes(icon.name)
)
...
// copy the chosen .svg files from the @mdi/svg package to the empty ./icon working folder here
const iconFolder = path.resolve(currentPath, 'icon')
if (fs.existsSync(iconFolder)) {
  fs.rmdirSync(iconFolder, { recursive: true, force: true })
}
...
metaJson.forEach(icon => {
  ...
  const srcFile = path.join(svgFolder, `u${icon.codepoint}-${icon.name}.svg`)
  if (fs.existsSync(srcFile)) {
    ...
    errors.push(`Invalid icon at "${srcFile}"`)
  }
})
if (errors.length) {
  errors.forEach(error => {
...
webfont({
  ...
  .then(result => {
    ...
    console.log(`- Generated woff, and woff2${argv.fontSvg && ' + svg'}`)
    generateIndex()
    generateSCSS()
    generateCSS()
    return result
